var dateUtil = {
    addHour: function (dates, num) {//增加小时，dates:'传入时间' num:增加小时数(可传小数) 返回传入时间格式类型，传入-号可做减法
        ...
        return dateUtil.composeDate(currentDate, format);
    },
    ...
    chooseDateFormat: function (str) {//str:传入日期 根据传入日期判断日期格式 返回日期格式 没有则返回空串
        ...
        } else if (dateUtil.YYYY_MM_DD) {
            return 'YYYY-MM-DD';
        } else if (dateUtil.YYYY_MM_DD_HH_MM) {
            return 'YYYY-MM-DD HH:MM';
        } else if (dateUtil.YYYY_MM) {
            return 'YYYY-MM';
        } else if (dateUtil.YYYY_MM_DD_HH) {
